\documentclass[AMA,STIX1COL,hidelinks]{WileyNJD-v2}
...
\lstdefinelanguage{Xtend}[]{Java}{
	...
	morecomment=[l]{//}, 
	morecomment=[s]{/*}{*/}, 
	moredelim=[il][\textcolor{annotationColor}]{$$},
%    moredelim=[is][\textcolor{annotationColor}]{\%\%}{\%\%}
	morestring=[b]",
	showspaces=false,
	tabsize=2, 
	numbers=left, 
...
\address[1]{
  ...
  \city{Bras\'{i}lia - DF, Brazil}.                        % city
%  \cny{Brazil}                                    % country
}

\corres{*Edna Dias Canedo. \email{ednacanedo@unb.br}}
...
%No plugin Maven desenvolvido estão contidas as especificações de diversos cenários de teste pré-definidos. Ao declarar o plugin no arquivo POM do projeto é necessário preencher algumas propriedades obrigatórias, sendo a mais importante a lista contendo os códigos QCs para os quais devem ser gerados os testes. Para cada QC declarado são executados todos os cenários especificados no plugin. Cada um desses cenários, quando executado, gera dois artefatos como saída: uma high-level rules no formato DSL e um arquivo de definição do cenário no formato do cucumber. Estes artefatos são gerados na fase de geração de código do ciclo de vida do Maven.
\textcolor{blue}{
...
%Após passar por outras fases do ciclo de vida do Maven, os artefatos de teste gerados são executados na fase de teste. Para cada cenário, definido nos arquivos do cucumber, é gerado um QDM para o QC declarado, usando as high-level rule geradas e definidas no cenário, e o resultado deste QDM é conferido de acordo com os dados previstos na definição do cenário do cucumber gerado. Foram criadas diversas combinações de cenários e para cada uma delas alguns dos valores das propriedades são gerados randomicamente, seguindo a técnica PBT.
\textcolor{blue}{
...
\begin{figure}[!ht] 
    \centering
	\includegraphics[scale=0.53]{img/processo.png}
	\caption{Tests Generation Process} 
	\label{fig:process}
\end{figure}
