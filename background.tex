...
PBT is a technique that is getting a lot of attention recently, it is strongly related to model-based testing, but where the focus is on generating individual tests automatically from specifications in the form of high-level property tests \cite{earle2014}. PBT is a strategy that leads to a family of automated testing tools. Generally, they are tools for generating and executing test cases based on specifications. These tools are typically implemented as libraries, allowing the developer / tester to write down program specifications, using a programming language of their choice, in the form of properties that must be satisfied. From these specifications, the tools automatically generate, execute, and verify the results of a large number of random test cases to verify that the properties are maintained \cite{castro2015}. There are a variety of PBT tools that have emerged over the years inspired by the original QuickCheck for Haskell \cite{claessen2000}, such as: JUnit-QuickCheck for Java, Theft for C, ScalaTest and ScalaCheck for Scala \cite{chepurnoy2018}.
...
PBT has three advantages over the manual test \cite{yatoh2014}. First, by writing only properties and omitting test data, developers can significantly reduce the size and complexity of the test code. Second, when running property-based tests against randomly generated entries, developers can find more bugs than manual tests, since property-based tests can check properties with many entries, thereby giving bugs is increased. Lastly, the properties can be seen as a mild formal specification of the program. Writing formal specifications promotes understanding of the specification and implementation and is effective in sharing the specifications among the team. It is important that this specification be executable. This executable specification can prevent specification and implementation incompatibility, which is generally the case if the specification is written to a separate document.
